    ...
    <View style={styles.container}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerSub}>Theo dõi tiến độ và hiệu suất học tập</Text>
        </View>
        ...
        {dailyLabels.length > 0 && (
          <View style={styles.chartCard}>
            <Text style={styles.chartTitle}>📈 Thời gian học (7 ngày gần nhất)</Text>
            <LineChart
        ...
        {quizLabels.length > 0 && (
          <View style={styles.chartCard}>
            <Text style={styles.chartTitle}>📝 Điểm kiểm tra</Text>
            <BarChart
        ...
        {pieData.length > 0 && (
          <View style={styles.chartCard}>
            <Text style={styles.chartTitle}>📚 Chủ đề đã học</Text>
            <PieChart
              data={pieData}
              width={chartWidth - 40}
              height={200}
        ...
        {aiAnalysis && (
          <View style={styles.analysisCard}>
            ...
            {aiAnalysis.strengths?.length > 0 && (
              <>
                <Text style={styles.sectionLabel}>💪 Điểm mạnh</Text>
            ...
            {aiAnalysis.weaknesses?.length > 0 && (
              <>
                <Text style={styles.sectionLabel}>📌 Cần cải thiện</Text>
            ...
            {aiAnalysis.optimalSchedule && (
              <>
                <Text style={styles.sectionLabel}>⏰ Lịch học tối ưu</Text>
                <Text style={styles.scheduleText}>{aiAnalysis.optimalSchedule}</Text>
              </>
            )}
          </View>
        )}
        ...
        {(!data || (data.totalSessions === 0 && data.totalQuizzes === 0)) && (
          <View style={styles.emptyCard}>
            ...
            <Text style={styles.emptyText}>Bắt đầu học và làm kiểm tra để xem thống kê tại đây!</Text>
          </View>
        )}
      </ScrollView>
    </View>
...
const styles = StyleSheet.create({
  ...
  analysisTitle: { color: '#FFF', fontSize: 18, fontWeight: '800', marginBottom: 12 },
  assessmentText: { color: '#B8B8D0', fontSize: 14, lineHeight: 22, marginBottom: 16 },
  sectionLabel: { color: '#FFF', fontSize: 15, fontWeight: '700', marginTop: 14, marginBottom: 8 },
  bulletRow: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 6 },
  bulletGreen: { marginRight: 8, fontSize: 14 },
  bulletRed: { marginRight: 8, fontSize: 14 },
  bulletBlue: { marginRight: 8, fontSize: 14 },
  bulletText: { color: '#B8B8D0', fontSize: 13, lineHeight: 20, flex: 1 },
  scheduleText: { color: '#8E8EAA', fontSize: 13, lineHeight: 20 },
  emptyCard: { alignItems: 'center', padding: 40, margin: 20, backgroundColor: '#1A1A2E', borderRadius: 16 },
  emptyIcon: { fontSize: 48, marginBottom: 12 },
  emptyTitle: { color: '#FFF', fontSize: 18, fontWeight: '700' },
  emptyText: { color: '#8E8EAA', fontSize: 14, textAlign: 'center', marginTop: 8, lineHeight: 22 },
});
